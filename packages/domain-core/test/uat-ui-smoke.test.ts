...
const browserEntry = readFileSync(
  resolve(root, "packages/domain-core/src/browser-entry.ts"),
  "utf8",
);
...
test("public service UI exposes required scenario inputs and safety boundary", () => {
  for (const id of [
    ...
    assert.match(compact, new RegExp(`id=["']${id}["']`));
  }
  assert.match(
    compact,
    /free, rules-first international trade and logistics decision-support service/i,
  );
  assert.match(compact, /preliminary decision support/i);
});
...
test("public service UI evaluates through the browser engine and renders module outputs", () => {
  ...
  for (const country of ["AE", "SA", "EG", "OM"])
    assert.match(browserEntry, new RegExp(`${country}:`));
});
